...
def alg_mine(filename):
    ...
    Algorithm:
    If it is a full moon, it buys 10 stocks it can and if it is a new moon
    it sells 10 stocks it can.
    ...
    for i in range(len(open_values)):
        if i == (len(open_values) - 1):
            ...
            price = open_values[i]
            current_day = dates[i]
            dateObj = datetime.strptime(current_day, "%Y-%m-%d")
            newMoon = datetime(2000, 1, 6)
            fullMoon = datetime(2000, 1, 22)
            moonCycle = timedelta(days=29, hours=12)
            if timedelta(days=1) <= (dateObj - newMoon) % moonCycle:
                cash_balance, stocks_owned = transact(cash_balance,
                                                      stocks_owned,
                                                      10,
                                                      price, sell=True)
            elif timedelta(days=1) <= (dateObj - fullMoon) % moonCycle:
                cash_balance, stocks_owned = transact(cash_balance,
                                                      stocks_owned, 10,
                                                      price, buy=True)
...
def main():
    ...
    print("The results are {0} stocks and ${1:0.2f}".format(alg1_stocks,
                                                            alg1_balance))
    ...
    print("The results are {0} stocks and ${1:0.2f}".format(alg2_stocks,
                                                            alg2_balance))
